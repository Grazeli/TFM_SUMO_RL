...
class EdgeVehiclesObservations(Observations):
    """
    ...
    per edge: # vehicles per step, speed, nb_lanes, blocked
    ...
    def _compute_observations(self):
        ...
        for key, item in self.edge_info.items():
            ...
            obs.extend(vehicles_count)
            obs.extend([average_speed, self.num_lanes[key], self.blocked[key]])
    ...
    def get_observations_size(self):
        ...
        """
        # Per edge: # vehicles count for each step + avg speed, num_lanes, blocked
        return len(self.edge_list) * (int(self.config.get(['environment', 'seconds_between_actions'])) + 3)
