...
class JunctionsObservations(Observations):
    """
    ...
    per edge: junction from and to, # vehicles per step, speed, nb_lanes, blocked
    ...
    def _precompute_observations(self):
        ...
        for item in root:
            ...
            if item.tag == 'connection' and item.attrib['from'][0] == ':':
                ...
                for j in [item.attrib['from'], item.attrib['to']]:
                    if not j in mapping_junction:
                        mapping_junction[j] = index
                        index += 1 


                self.data_precomputed[item.attrib['id']] = (mapping_junction[item.attrib['from']], mapping_junction[item.attrib['to']])
    ...
    def _compute_observations(self):
        ...
        for key, item in self.edge_info.items():
            ...
            obs.extend(self.data_precomputed[key])
            obs.extend([sum(vehicles_count) ,average_speed, self.num_lanes[key], self.blocked[key]])
