...
import state_representation.edgeAnalyticsObservations as stateEdgeAnalytics
import state_representation.vehiclesObservations as stateVehicles
import state_representation.junctionsObservations as stateJunctions
...
class SUMORLEnvironment(gym.Env):
    ...
    def __init__(self, config: Configuration, sumo_configuration_path: str):
        ...
        if self.config.get(['sumo_command', 'gui-activated']) == 'True':
        ...
        if environment_config['state_representation'] == 'edgeAnalytics':
            self.state_representation = stateEdgeAnalytics.EdgeAnalyticsObservations(self.traci, self.config)
        elif environment_config['state_representation'] == 'edgeVehicles':
            self.state_representation = stateVehicles.EdgeVehiclesObservations(self.traci, self.config)
        elif environment_config['state_representation'] == 'junctions':
            self.state_representation = stateJunctions.JunctionsObservations(self.traci, self.config)
    ...
    def step(self, action: int):
        ...
        state_observation, speed_list = self.state_representation.get_observations()
        reward = reward_function.compute_reward(traci, self.config.get(['rl', 'reward_function']), self.config, speed_list)

        #print('Speed list {}'.format(max(speed_list)))
    ...
    def reset(self):
        ...
        if self.starting_step:
            ...
            aux, _ = self.state_representation.get_observations()
    ...
    def step_event(self):
        ...
        :return: None
        """
        event_edges = self.config.get(['event', 'edge'])
        sumo_event.eventBlockEdges(self.traci, event_edges)
        self.state_representation.blocked_route(event_edges)
